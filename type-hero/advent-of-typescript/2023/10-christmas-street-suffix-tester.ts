...
// There were hopes among some reindeer to introduce a validation library this year, but there was simply too much infighting.
// It's kindof a mess. You see..
...
// They both had popular webpack plugins in the past that are no longer used by anyone and now they're a little bitter about change (in general).
// Then you have Prancer.
// Prancer doesn't see the point in validation.
// Prance feels that validating inputs pollutes the code with type gymnastics that add ever so little joy to the development experience.
// Yep.
// Even the reindeer have one of these on their team in 2023.
...
// The Type's API
// So, for this year.. nothing fancy.
// We'll have to just write a StreetSuffixTester from scratch.
...
// Answer
type StreetSuffixTester<String extends string, Suffix extends string> = String extends `${string}${Suffix}`
  ? true
  : false;
